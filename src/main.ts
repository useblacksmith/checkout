...
async function cleanup(): Promise<void> {
  ...
  if (exposeId && stickyDiskKey) {
    ...
    // The bounded repack is reported under the `gc` operation: it is the
    // pack-consolidation step of the mirror lifecycle, whatever git verb
    // performs it.
    if (
      cleanupResult &&
      mirrorPath &&
      !cleanupResult.maintenanceResult.skipped
    ) {
      await mirrorTelemetry.reportMaintenance(
        mirrorTelemetry.maintenanceRunFromResult(
          'gc',
          stickyDiskKey,
          cleanupResult.maintenanceResult
        )
      )
    }
  }
